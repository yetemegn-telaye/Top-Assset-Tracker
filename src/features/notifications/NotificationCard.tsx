    ...
    return (
        <div className="bg-background border flex gap-4 items-center p-4 rounded-lg shadow-lg hover:bg-accent-lighter ">
            ...
            <div className="flex flex-col gap-1">
                <h5 className={`text-sm text-${(notification.message.includes('delayed'))? 'error-light':(notification.message.includes('waiting'))? 'yellow-500':'gray-500'} ${(notification.message.includes('waiting'))? 'animate-bounce': ''}`}>{notification.message}</h5>
